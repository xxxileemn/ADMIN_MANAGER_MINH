...
const OrderTable: React.FC<OrderTableProps> = ({ orders, onUpdateStatus, onPrintOrder }) => {
  ...
  // Tính toán tổng hợp doanh thu cho danh sách đang lọc
  ...
  return (
    <div className="space-y-4 md:space-y-6 pb-40">
      ...
      {/* 2. Tổng hợp doanh thu - MỚI THÊM */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
           <div className="flex items-center gap-2 mb-2">
              <span className="text-emerald-500 bg-emerald-50 p-1.5 rounded-lg text-sm">💰</span>
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Tổng thực thu</span>
           </div>
           <p className="text-lg font-black text-slate-900 leading-none">{formatCurrency(summaryStats.totalRevenue)}</p>
        </div>
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
           <div className="flex items-center gap-2 mb-2">
              <span className="text-rose-500 bg-rose-50 p-1.5 rounded-lg text-sm">🎟️</span>
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Tổng giảm giá</span>
           </div>
           <p className="text-lg font-black text-slate-900 leading-none">{formatCurrency(summaryStats.totalDiscount)}</p>
        </div>
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
           <div className="flex items-center gap-2 mb-2">
              <span className="text-indigo-500 bg-indigo-50 p-1.5 rounded-lg text-sm">📦</span>
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Số đơn hàng</span>
           </div>
           <p className="text-lg font-black text-slate-900 leading-none">{summaryStats.orderCount} đơn</p>
        </div>
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
           <div className="flex items-center gap-2 mb-2">
              <span className="text-amber-500 bg-amber-50 p-1.5 rounded-lg text-sm">📊</span>
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Trung bình/đơn</span>
           </div>
